...
public class CreateEndpointTest {
	...
	private final String insertUserJSON = "{\"username\":\"a\","
	...
	public void successOnInit() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.post(ENDPOINT_RESOURCE_BASE_URL).contentType(MediaType.APPLICATION_JSON)
				...
				.andExpect(jsonPath("$.numberOfUsers").exists()).andExpect(jsonPath("$.numberOfUsers").value(4))
				...
				.andExpect(jsonPath("$.users[3].idUser").exists())
				.andExpect(jsonPath("$.users[3].username").exists())
				.andExpect(jsonPath("$.users[3].username").value("a")).andExpect(jsonPath("$.users[3].email").exists())
				.andExpect(jsonPath("$.users[3].email").value("b.b@b.b"))
				.andExpect(jsonPath("$.users[3].password").exists())
				.andExpect(jsonPath("$.users[3].password").value("b"))
