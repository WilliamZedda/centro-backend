...
public class FindAllAlertsEndpointTest {
	...
	public void successOnInit() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get(ENDPOINT_RESOURCE_BASE_URL).accept(MediaType.APPLICATION_JSON))
				...
				.andExpect(jsonPath("$.alerts[0].idDeviceFk").value("telecamera"))
				.andExpect(jsonPath("$.alerts[0].timestamp").exists())
				.andExpect(jsonPath("$.alerts[0].timestamp").value("2000-12-12 10:10:10"))
				.andExpect(jsonPath("$.alerts[0].code").exists())
				.andExpect(jsonPath("$.alerts[0].code").value(13))
